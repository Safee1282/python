...
def msg():
    msgBox=messagebox.showinfo(
        "Alert","Do you want to calculate the deonomination count?")
    if msgBox =='ok':
        topwin()
...
def topwin():
    top=Toplevel() 
    top.title('Denomination calcular')
    top.configure(bg='light grey')
    top.geometry("600x350+50+50")

    label=Label(top,text='Enter total number',bg='light grey')
    entry=Entry(top)
